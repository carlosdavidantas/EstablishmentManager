...
//            "create table [id_of_product_addons] ([id] int primary key identity(1,1), [id_product] int foreign key references product(id), [id_product_addons] int foreign key references product_addons(id));" +
//            "create table [id_of_observation] ([id] int primary key identity(1,1), [id_product] int foreign key references product(id), [id_product_observation] int foreign key references product_observation(id));" +
...
//            "create table [id_of_product_on_group] ([id] int primary key identity(1,1), [id_of_group_of_product] int foreign key references group_of_product(id), [id_product] int foreign key references product(id));" +
//            "create table [promotion] ([id] int primary key identity(1,1), [id_product] int foreign key references product(id), [id_group_of_product] int foreign key references group_of_product(id), [name] varchar(100), [description] varchar(100), [sell_price] decimal, [category] varchar(100), [created] date);" +
...
//            "create table [delivery] ([id] int primary key identity(1,1), [id_deliveryman_employee] int foreign key references employee(id), [id_orders] int, [id_client] int, [tax_value] decimal, [creation_date] date, [creatin_time] time, [time_deliveryman_arrived] time, [schedule_date] date);" +
...
//            "create table [transaction] ([id] int primary key identity(1,1), [id_payment_method] int foreign key references payment_method(id), [id_order] int foreign key references [order](id), [date] date, [hour] time);" +
...
//            "create table [id_client_telephone] ([id] int primary key identity(1,1), [id_client] int foreign key references client(id), [id_telephone] int foreign key references client_telephone(id));" +
