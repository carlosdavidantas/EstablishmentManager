namespace EstablishmentManagerLibrary.Database
{
    ...
    {
        const string DATABASENAME = "EstablishmentManager"; // Change "EstablishmentManager" if you want another database name.
        const string CONNECTIONSTRING = @"Server=localhost\SQLEXPRESS;Database=master;Trusted_Connection=True";

        public static string Database_name { get => DATABASENAME; }
        ...
        public static bool CreatDataBase()
        {
            void CreateDB()
            {
                string qString = $"CREATE DATABASE {Database_name} ON PRIMARY " +
                 $"(NAME = {Database_name}_Data, " +
                 $@"FILENAME = 'C:\Program Files\Microsoft SQL Server\MSSQL16.SQLEXPRESS\MSSQL\DATA\{Database_name}.mdf', " + //Check if your file location is the same and the version of the MSSQL16.SQLEXPRESS.
                 "SIZE = 2MB, MAXSIZE = 10MB, FILEGROWTH = 10%)" +
                 $"LOG ON (NAME = {Database_name}_Log, " +
                 $@"FILENAME = 'C:\Program Files\Microsoft SQL Server\MSSQL16.SQLEXPRESS\MSSQL\DATA\{Database_name}.ldf', " + //Check if your file location is the same and the version of the MSSQL16.SQLEXPRESS.
                 "SIZE = 1MB, " +
                 "MAXSIZE = 5MB, " +
                 "FILEGROWTH = 10%);";
                Query.Execute(qString, CONNECTIONSTRING);
            }
            CreateDB();

            string queryString = $"use {Database_name};" +
            ...
            Query.Execute(queryString, CONNECTIONSTRING);
            return true;
        }
    }
}
